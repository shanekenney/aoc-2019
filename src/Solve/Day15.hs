...
executeCommand move droid@(RepairDroid { memory, position }) plane =
  let memoryMoveStatus = execute $ memory { inputs = [movementCommandToInt move] }
      ...
      newPlane = Map.insert (getCoordinate move position) (movementStatusToTile moveStatus) plane
      newDroid = droid 
        { memory = memoryMoveStatus 
        , position = nextPosition move moveStatus position
        }
   in (moveStatus, newDroid, newPlane)
...
exploreHull :: Plane -> RepairDroid -> Plane
exploreHull plane droid@(RepairDroid { position }) = 
  foldl explore plane $ possibleMoves plane position
  where
    explore plane' moveCommand = 
      let (status, newDroid, newPlane) = executeCommand moveCommand droid plane'
       in case status of
            Moved -> exploreHull newPlane newDroid
            _ -> newPlane

shortestPath :: Plane -> Coordinate -> Coordinate -> Int -> Maybe Int
shortestPath plane previousPosition position steps = foldl shortest Nothing neighbours
  where
    shortest min' coordinate =
      let thisLength = case Map.lookup coordinate plane of
                         Just Goal -> Just (steps + 1)
                         Just Empty -> shortestPath plane position coordinate (steps + 1)
                         _ -> error "Should only be accessible neighbours."
       in minMaybe min' thisLength

    neighbours = delete previousPosition $ foldl toNeighbour [] [North, South, East, West]
    toNeighbour coords move = 
      let neighbour = getCoordinate move position
      in case Map.lookup neighbour plane of
           Just Empty -> neighbour:coords
           Just Goal -> neighbour:coords
           _ -> coords

farthestPoint :: Plane -> Coordinate -> Coordinate -> Int -> Int
farthestPoint plane previousPosition position steps = foldl farthest steps neighbours
  where
    farthest max' coordinate =
      let thisLength = case Map.lookup coordinate plane of
                         Just Empty -> farthestPoint plane position coordinate (steps + 1)
                         _ -> error "Should only be accessible neighbours."
       in max max' thisLength
...
partOne inputStr = shortestPath plane startPosition startPosition 0
  where
    startPosition = (0,0)
    plane = exploreHull (Map.singleton startPosition Empty) droid
    droid = RepairDroid
...
partTwo :: String -> Int
partTwo inputStr = farthestPoint plane goalPosition goalPosition 0
  where
    startPosition = (0,0)
    (goalPosition, _) = head $ filter (\(_, tile) -> tile == Goal) $ Map.toList plane 
    plane = exploreHull (Map.singleton startPosition Empty) droid
    droid = RepairDroid
      { memory = initMemory $ programVector $ inputStr
      , position = startPosition
